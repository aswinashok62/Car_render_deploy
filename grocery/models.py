...
class Supplier(models.Model):
    ...
    company = models.CharField(max_length=100, default='Default Company Name')  # Set a default value
...
class Cart(models.Model):
    ...
    def __str__(self):
        return f"{self.profile.user.username} - {self.product.name} ({self.quantity})"
...
class ProductReport(models.Model):
    ...
    def __str__(self):
        return f"{self.product.name} - Bookings: {self.total_bookings}, Deliveries: {self.total_deliveries}"

    @staticmethod
    def update_reports():
        # Aggregate data for each product
        products_data = Booking.objects.values('product').annotate(
            total_bookings=Count('id'),
            total_deliveries=Sum('quantity')
        )

        # Update or create reports
        for data in products_data:
            product = Product.objects.get(id=data['product'])
            report, created = ProductReport.objects.get_or_create(product=product)
            report.total_bookings = data['total_bookings']
            report.total_deliveries = data['total_deliveries']
            report.save()
